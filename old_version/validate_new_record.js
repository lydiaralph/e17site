...
function validateBibleRef(ch_start,ch_end,verse_start,verse_end) {
    ...
    for(var arg=0; arg < arguments.length; arg++){
        ...
        if(!field.value) {
            ...
            switch (arg) {
                case '0': brfail += " chapter start\n"; break;
                case '1': brfail += " chapter end.\n"; break;
                case '2': brfail += " verse start.\n"; break;
                case '3': brfail += " verse end.\n"; break;
                default: break;
            }
        }
    }
    ...
    if (brfail != "") {
        writeError(document.getElementById("ch_v"), "Required field");
        return brfail;
    }
...
function validatePreacher(known_preacher, new_preacher) {
    if( known_preacher == "" && new_preacher == "") return "Please select a preacher.\n"
//    if( preacher == "") return "Please select a preacher.\n"
    return ""
}

function validateSeries(known_series, new_series) {
    if( known_series == "" && new_series == "") return "Please select a series.\n"
    return ""
}

function validateSermonTitle(sermon_title) {
    if( sermon_title == "") return "Please enter a sermon title.\n"
    return ""
}

function validateFile(fileName){
    if ( fileName == "") return "Please upload an mp3 file or a PDF file.\n";
